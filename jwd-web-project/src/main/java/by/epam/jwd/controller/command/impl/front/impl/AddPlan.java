...
public class AddPlan implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int planCountPlaces = Integer.parseInt(request.getParameter(FormParameter.COUNT_PLACES).toString());
		...
		try {
			...
			if (flag) {
				session.setAttribute(SessionParameter.SUCCESS_ADD_PLAN, SessionParameter.SUCCESS_ADD_PLAN);
			} else {
				session.setAttribute(SessionParameter.UNSUCCESS_ADD_PLAN, SessionParameter.UNSUCCESS_ADD_PLAN);	
			}
